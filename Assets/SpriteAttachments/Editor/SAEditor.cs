            ...
            if (sprites.Count > selectedSprite)
            {
                foreach (SerializedSpriteAttachmentSprite attachmentSprite in SpriteSheet.AttachmentSprites)
                {
                    foreach (SerializedSpriteAttachment attachment in attachmentSprite.Attachments)
                    {
                        ...
                        // Account for margin or whatever it is pushing stuff out of whack.
                        Vector2 editorPos = attachment.EditorPosition;
                        ...
                        float percentX = (editorPos.x - pivot.x + iconSize.x) / sW;
                        float percentY = ((editorPos.y - sH) - pivot.y + iconSize.y) / sH;

                        attachment.NormalizedPosition = new Vector2(percentX, -percentY);
                        //Debug.Log($"Editor: {editorPos}\nNormalized: ({attachment.NormalizedPosition.x}, {attachment.NormalizedPosition.y})");
                    }
                }
            }
        }
